<template>
    <div id="allworld">
        <div class="allworld">
           <div class="allworldmap">
              <div id="world">
              </div>
              </div>
        </div>
        <div class="flex_b allworld_b">
             <div class="allworldmap_tips">
                    <div class="flex_b">
                        <div class="flex_s_c">
                            <div class="tips_box1"></div>
                            <span class="opacite1">已开发</span>
                         </div>
                          <div class="flex_s_c">
                            <div class="tips_box2"></div>
                            <span class="opacite1">待开发开发</span>
                         </div>       
                    </div>
             </div>
            <div class="flex_s allworld_bitem">
              <div> <img src="../assets/img/world1.png" alt=""></div>
              <div class="flex_f_f allworld_btext">
                    <div class="flex_s color5 fontsize1 allworldnumber"> <span class="allworldnumbertexxt">{{nodesdistribution.renderWork}}</span>
                          <div class="add_allworld color6" :class="addtype1?'addsnumber':''">+1</div>
                    </div>
                  <div class="fontsize2">WORKER</div>         
              </div>
              </div>
              <div class="flex_s allworld_bitem">
              <div> <img src="../assets/img/world2.png" alt=""></div>
              <div class="flex_f_f allworld_btext">
                    <div class="flex_s color6 fontsize1 allworldnumber"> <span>{{nodesdistribution.renderUser}}</span>
                          <div class="add_allworld color7" :class="addtype1?'addsnumber':''">+1</div>
                    </div>
                  <div class="fontsize2">USER</div>         
              </div>
              </div>
            <div class="flex_s allworld_bitem">
              <div> <img src="../assets/img/world3.png" alt=""></div>
              <div class="flex_f_f allworld_btext">
                    <div class="flex_s color7 fontsize1 allworldnumber"> <span>{{nodesdistribution.renderGpu}}</span>
                          <div class="add_allworld color8" :class="addtype1?'addsnumber':''">+1</div>
                    </div>
                  <div class="fontsize2">计算机GPU</div>         
              </div>
              </div>
             <div class="flex_s allworld_bitem">
              <div> <img src="../assets/img/world4.png" alt=""></div>
              <div class="flex_f_f allworld_btext">
                    <div class="flex_s color8 fontsize1 allworldnumber"> <span class="allworldnumbertexxt">{{Math.floor(nodesdistribution.renderTime*10)/10||0}}h</span>
                          <div class="add_allworld color5 add_allworld4" :class="addtype1?'addsnumber':''">+1</div>
                    </div>
                  <div class="fontsize2">累计时数</div>         
              </div>
              </div>                                                                                
        </div>
    </div>
...
import { setInterval } from 'timers';
    //   import options from './common/map'  //引入配置项
export default {
  created() {
 ...
 //全球区块链节点分布数据 
        ...
        let _that=this
         this.$axios.post(this.$api.nodesdistribution).then(rsp=>{
              console.log(rsp.data,'555555');     
             _that.nodesdistribution=rsp.data               
        })              
    },
     //全球区块链节点地址分布 
            getblockaddress(){
        let _that=this
         this.$axios.post(this.$api.blockaddress).then(rsp=>{
              console.log(rsp.data,'blockaddress');     
             _that.blockaddress=rsp.data               
        })              
    },
drawLine() {
      let _that = this;
      let myChart = this.$echarts.init(document.getElementById("world"));
      // 绘制图表
      myChart.showLoading();
...
var geoCoordMap = {
    ...
    "shenzhen":[114.063977,22.549791],
    "xiamen":[118.102184,24.495142],
    "suzhou":[120.598161,31.313684],
    "chengdu":[104.074631,30.662533],
    "hangzhou":[120.207461,30.257325],
};
...
myChart.setOption({
     backgroundColor: _that.$echarts.graphic.RadialGradient(0.5, 0.5, 0.4, [{
        offset: 0,
        color: '#050719',
         backgroundColor:'#11113f',
    }, {
        offset: 1,
        color: '#404a59',
         backgroundColor:'#11113f',
...
.addsnumber{
   animation: gotop 3s linear 0s infinite;
}
@keyframes gotop{
    0%{
        bottom: -0.1rem; 
